...
main() {
  ...
  if $show_border_contrast; then
    # tmux set-option -g pane-active-border-style "bg=${dark_gray},fg=${light_purple}"
    tmux set-option -g pane-active-border-style "bg=${dark_gray},fg=${pink},bold"
  else
    tmux set-option -g pane-active-border-style "fg=${dark_purple}"
  fi
  tmux set-option -g pane-border-style "fg=${gray}"
  # Nov-21-2025: It doesn't work
  # tmux set-option -g border-chars "═║╔╗╚╝╠╣╦╩╬"
  ...
  if $show_powerline; then
    if $show_edge_icons; then
      tmux set-option -g status-left "#[bg=${bg_color},fg=${light_purple},bold]#{?client_prefix,#[fg=${orange}],}${show_right_sep}#[bg=${light_purple},fg=${dark_gray}]#{?client_prefix,#[bg=${orange}]#[fg=${dark_gray}],} ${left_icon} #[fg=${light_purple},bg=${bg_color}]#{?client_prefix,#[fg=${orange}],}${left_sep} "
    else
      tmux set-option -g status-left "#[bg=${dark_gray},fg=${light_purple}]#[bg=${light_purple},fg=${dark_gray}]#{?client_prefix,#[bg=${orange}]#[fg=${dark_gray}],} ${left_icon} #[fg=${light_purple},bg=${bg_color}]#{?client_prefix,#[fg=${orange}],}${left_sep}"
    fi
    powerbg=${bg_color}
  else
    tmux set-option -g status-left "#[bg=${light_purple},fg=${dark_gray}]#{?client_prefix,#[bg=${orange}]#[fg=${dark_gray}],} ${left_icon}"
  fi
  ...
  for plugin in "${plugins[@]}"; do
    ...
    if $show_powerline; then
      if $show_empty_plugins; then
        tmux set-option -ga status-right " #[fg=${!colors[0]},bg=${powerbg},nobold,nounderscore,noitalics]${right_sep}#[fg=${!colors[1]},bg=${!colors[0]}] $script$right_edge_icon"
      else
    tmux set-option -ga status-right "#{?#{==:$script,},,#[fg=${!colors[0]},bg=${powerbg},nobold,nounderscore,noitalics] ${right_sep}#[fg=${!colors[1]},bg=${!colors[0]}] $script$right_edge_icon}"
    fi
      powerbg=${!colors[0]}
    else
      if $show_empty_plugins; then
  ...
  # Add final edge arrow for status-right
  if $show_powerline && $show_edge_icons; then
    tmux set-option -ga status-right " #[fg=${powerbg},bg=${bg_color}]${show_left_sep}"
  fi
